...
Этот файл — что подключить для **боевого B2B с реальными лидами**.

> **Текущее состояние — полностью валидное качественное портфолио.** Демо-режим (формы работают, заявки пишутся локально в `uploads/leads.json`, код production-ready) — самодостаточный финальный стейт. Ниже — **опциональные** внешние интеграции; они не нужны для портфолио, только если проект пойдёт в реальную эксплуатацию.

---

## 🔴 Для боевого B2B (опционально — для портфолио НЕ нужно)
